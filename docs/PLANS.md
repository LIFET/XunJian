...
- App 测试 183 项执行（181 通过、2 项发布门禁跳过）、Process 34/34、Debug 与 Universal Developer ID Release 构建通过。新 DMG 已提交公证任务 `82295e7c-e477-45c8-875a-b68451aa1968`，Accepted 后完成 staple 与最终校验。

## 风险

- 外部应用进行的跨来源移动仍可能获得新身份；寻简内置“移动到”已迁移分类与搜索索引，文件身份仍由卷与资源标识决定，不引入版本系统。
- 内容哈希的使用范围已明确放开，但仅限“查找重复文件”这一由用户显式触发的一次性分析：先按文件大小分组，只对可能重复的组做 SHA-256 流式分块计算，跳过超过 128MB 的文件，结果不落库、不参与文件身份判定。除此之外不得把内容哈希引入索引、身份识别或同步逻辑。
