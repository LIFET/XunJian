...
Phase 8S 四个核心页面的可见层级重塑已在独立 `codex/ui-polish-0.1.2` worktree 完成；0.1.1 发布分支与待公证 DMG 未改动。版本源暂仍为 0.1.1（build 2），未运行真实 OAuth/AI 或文件扫描。

## 已完成内容

- 首页增加本地搜索导引面板、紧凑授权空状态与分区说明；所有文件将筛选工具和空状态收拢为明确表面；分类增加用途说明和完整“新建分类”操作；设置增加导引区及 760pt 可读宽度。业务动作、数据源与路由未改。
- 新增无模糊、无阴影的 `InsetSurface`，只用于页面导引和空状态；浅色/深色、窄/宽窗口已通过隔离 Debug App 真实截图复核，分类按钮压缩问题在实机发现后修正。
- Phase 8S 验收：arm64 Debug build 成功；App 363 项（2 项按设计跳过）与 OAuth Process 38 项共 401 项、0 失败；`git diff --check` 通过。隔离预览通过测试环境使用临时数据库，未触发真实 OAuth、AI、扫描或文件操作。
