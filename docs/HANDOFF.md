...
Phase 8 的代码实现与离线验收已完成；未提交、未生成 DMG、未运行真实 OAuth/AI。Sparkle EdDSA 私钥已存登录钥匙串，公钥已接入；真实 HTTPS appcast 地址仍需发布方提供。
...
- 更新：固定 Sparkle 2.9.5，设置页和应用菜单提供检查更新、自动检查开关；密钥账户为 `com.xingmingbo.XunJian`，私钥未导出。缺少 HTTPS feed 时保持禁用且零请求。
...
- 配置真实 `XUNJIAN_UPDATE_FEED_URL` 后，需用预发布签名 appcast 验证升级、伪造包拒绝和失败恢复。
