...
enum SettingsPage: String, CaseIterable, Identifiable {
  ...
  var title: String {
    switch self {
    case .general: AppLanguage.localized("通用", english: "General")
    case .files: AppLanguage.localized("文件与索引", english: "Files & Index")
    case .ai: AppLanguage.localized("AI 服务", english: "AI Services")
    case .about: AppLanguage.localized("关于", english: "About")
    }
  }
  var summary: String {
    switch self {
    case .general:
      AppLanguage.localized(
        "这里的更改会自动保存。",
        english: "Changes here are saved automatically.")
    case .files:
      AppLanguage.localized(
        "选择搜索范围，设置要跳过的文件和文件夹。",
        english: "Choose where to search and which files and folders to skip.")
    case .ai:
      AppLanguage.localized(
        "登录账号或填写 API Key，再选择模型。修改配置后需要保存。",
        english: "Sign in or enter an API key, then choose a model. Save any configuration changes.")
    case .about:
      AppLanguage.localized("查看当前版本，检查更新。", english: "View the current version and check for updates.")
    }
  }
}
...
struct SettingsView: View {
  ...
  @State private var selectedProvider: AIProviderKind = .codex
  ...
  var body: some View {
              ...
              Button { selectedPage = page } label: {
                Label(page.title, systemImage: page.symbol)
                  ...
                  .overlay(alignment: .bottom) {
                    if selectedPage == page { Rectangle().fill(.primary).frame(height: 2) }
                  }
              }
              .buttonStyle(.plain)
    ...
    .background(AppVisualTheme.resolve(visualTheme).palette(for: colorScheme).canvas)
    .id(language)
    ...
    .confirmationDialog(
      ...
      Text(
        AppLanguage.localized(
          ...
          english:
            "This removes XunJian’s saved access and local index. The original folder and its files stay on disk."
        )
      )
    }
  }
  ...
  private var settingsForm: some View {
    Form {
      switch selectedPage {
      case .general: generalSettings; browsingSettings
      case .files: locationSettings; exclusionSettings; indexSettings
      case .ai: aiSettings
  ...
  private var generalSettings: some View {
    Section(AppLanguage.localized("外观与行为", english: "Appearance & Behavior")) {
  ...
  private var locationSettings: some View {
    Section(AppLanguage.localized("搜索内容", english: "Search Content")) {
      Toggle(
        AppLanguage.localized(
          "显示以点号开头的隐藏文件",
      ...
      Text(
        verbatim: AppLanguage.localized(
          ...
          english:
            "By default, files and folders beginning with “.”, such as .DS_Store, are not indexed. Changing this setting rescans your folders."
        )
      )
      .font(.callout)
      .foregroundStyle(.secondary)
      ...
      Text(
        verbatim: AppLanguage.localized(
          ...
          english:
            "File contents stay in the local index for full-text search. Turning this off clears stored contents immediately. Content is sent to the current AI only when you explicitly use an AI file action."
        )
      )
      .font(.callout)
      .foregroundStyle(.secondary)
    ...
    Section(AppLanguage.localized("扫描范围", english: "Scan Scope")) {
      Picker(
          ...
          if let source = appModel.wholeMacSource {
            ...
            Text(
              verbatim: AppLanguage.localized(
                ...
                english:
                  "Only the Entire Mac scope is active. Added folders are retained and resume when you switch back."
              )
            )
            .font(.caption)
            .foregroundStyle(.secondary)
            ...
            Text(
              verbatim: AppLanguage.localized(
                ...
                english:
                  "Entire Mac scanning requires selecting the startup disk and granting Full Disk Access when needed. Scanning does not start before authorization."
              )
            )
            .font(.caption)
            .foregroundStyle(.secondary)
          ...
          Label(
            showsAllAuthorizedFolders
              ...
              : AppLanguage.localized(
                ...
                english:
                  "Show \(appModel.selectedFolderSources.count - Self.collapsedAuthorizedFolderLimit) More Folders"
              ),
      ...
      if appModel.selectedFolderSources.count > 1 {
        Button(AppLanguage.localized("重新扫描这些文件夹", english: "Rescan These Folders")) {
          appModel.refreshAllSources()
        }
        .disabled(
          appModel.isScanning
            || !appModel.selectedFolderSources.contains(where: {
  ...
  private var aiSettings: some View {
    Section {
      ...
      Picker(AppLanguage.localized("管理服务", english: "Manage Service"), selection: $selectedProvider) {
        ForEach(AIProviderKind.allCases) { kind in
          Text(verbatim: providerTitle(kind)).tag(kind)
        }
      }
      .pickerStyle(.menu)
      AIProviderSettingsRow(kind: selectedProvider, isDetail: true)
        .id(selectedProvider)
    } header: {
      Text(AppLanguage.localized("连接与模型", english: "Connections & Models"))
    } footer: {
      Text(
        verbatim: AppLanguage.localized(
          "凭据只保存在当前用户的寻简本地数据目录，不会写入 App 包或随 App 分享。普通搜索始终在本地完成。",
          english:
            "Credentials stay in XunJian's local data folder for the current user and are never included in the app bundle or shared with it. Regular search always stays on your Mac."
        )
      )
    }
  }
    ...
    Section(AppLanguage.localized("扫描排除", english: "Scan Exclusions")) {
      Text(
        verbatim: AppLanguage.localized(
          ...
          english:
            "Scans always skip build and cache folders such as .git, node_modules, DerivedData, and Caches. Add file or folder names to exclude; changes trigger a rescan."
        )
      )
      .font(.caption)
      .foregroundStyle(.secondary)
      ...
      VStack(alignment: .leading, spacing: 6) {
        ...
        Text(
          verbatim: AppLanguage.localized(
            ...
            english:
              "Use this when search results look wrong. It only regenerates search data and compacts the database — no files, locations, or categories are removed."
          )
        )
        .font(.caption)
        .foregroundStyle(.secondary)
  ...
  private var aboutSettings: some View {
    Section(AppLanguage.localized("版本与更新", english: "Version & Updates")) {
      LabeledContent(
      ...
      if !updateCoordinator.isConfigured {
        Text(
          verbatim: AppLanguage.localized(
            "此版本未配置自动更新，请手动安装新版本。",
            english:
              "Automatic updates are not configured for this build. Install a newer version manually."
          )
        )
        .font(.caption)
        .foregroundStyle(.secondary)
  ...
  private func sourceIdentity(_ source: FileSource) -> some View {
    HStack(spacing: 12) {
      Image(
        systemName: source.accessState == .available ? "folder" : "folder.badge.exclamationmark"
      )
      .foregroundStyle(
        Color(
  ...
  private var activeProviderSummary: String {
    guard let kind = ai.activeProviderKind,
      let mode = ai.activeAuthenticationMode
    else {
